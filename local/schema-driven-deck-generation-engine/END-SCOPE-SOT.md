...
Related: [`PROGRESS.md`](PROGRESS.md) · [`../../docs/PRODIE-IDEATION-SPEC.md`](../../docs/PRODIE-IDEATION-SPEC.md) (**historical** Prodie ideation — not sales-mcp runtime contract) · [`CLEANUP-TODO.md`](CLEANUP-TODO.md) · [`NEXT-STEPS.md`](NEXT-STEPS.md) · Jira Creation [PI-2516](https://fortune.atlassian.net/browse/PI-2516) · Shelved stylist [PI-2754](https://fortune.atlassian.net/browse/PI-2754)
...
There is **no `propose_mix` MCP tool**. In-repo `LogicGuideEngine` (`ingestion/logic_guide/`) is **reference/test code only**, not associate runtime. **`confirm_mix`** is optional legacy when the caller sends only name+category and needs price/cadence hydration from GTM.
...
**Not the end state:** sales-mcp ideating or proposing products · RAG/Titan similarity for product pages · Cursor vision stylist as primary gate without QA rail · industry Category_Presentation_* matrix as primary spine · Claude inventing product slide copy · Prodie auto-locking a funded Media Mix through this MCP.
...
Guide **Media Mix Logic** (auto-fund a package) is **reference only**. The locked `confirmed_products[]` is the mix. The in-repo `LogicGuideEngine` is for **tests and fixtures**, not associate runtime.
...
| **RAG is not Creation-critical** | Titan/embeddings may linger for research tools (`search_decks`, etc.); not the product-page selector. |
| **Stylist (Phase B) → QA rail** | Headless Cursor QA rescopes shelved [PI-2754](https://fortune.atlassian.net/browse/PI-2754). See `docs/DECK-QA-ARCHITECTURE.md`. |
...
| Logic Guide in repo | I2 reference/tests only — not runtime |
...
| Slim RAG/stylist-era weight | CLEANUP-TODO |
...
7. `ingestion/logic_guide/` is reference/test code — do not wire it as associate runtime.
