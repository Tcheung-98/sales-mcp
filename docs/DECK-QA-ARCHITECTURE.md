...
> **Repo:** `sales-mcp` only. Upstream Pitch Deck Builder / product lock (PI-2350) is a separate track.  
...
```text
Locked DeckSchema (complete payload from upstream caller)
...
4. **No `propose_mix` MCP tool** — sales-mcp does not ideate products.
...
`run_headless_cursor_qa(package, *, timeout_s)` is an **in-process helper in `scripts/run_deck_qa.py`**, imported by `generator.py`; the CLI in that file is a thin `argparse` wrapper over the same helper. Do not shell out to `python scripts/run_deck_qa.py` from `build()` — `scripts/` is not in `packages` in `pyproject.toml`, so it is not importable or resolvable from an installed wheel or the Docker image without extra path work. PR-E must either add `scripts` to `[tool.setuptools] packages` with an `__init__.py`, or place the helper under `ingestion/deck_qa_agent.py` and keep `scripts/run_deck_qa.py` as a CLI shim. **Decide this in PR-E, not PR-F.**
...
>
> The prices also contradict the rate card the repo does have: `REPRESENTATIVE_PRICING_ROWS` lists CEO Daily at `$5,000/day` and Crown Unit at `$25,000`, not `$75,000` / `$85,000`. That is fine for a hand-built `DeckSchema` (C2 only checks that the mix sums to the stated budget) but it is **not** a mix the optional legacy `confirm_mix` would produce, since `confirm_mix` derives price from the inventory workbook. The primary path sends full `confirmed_products[]` directly. Do not present this as a realistic end-to-end fixture.
>
