...
| 2026-09-08 | PR-F: rail wired into `build()` behind `DECK_QA_ENABLED`, `assemble_skeleton` refactored onto `plan_pitch_sequence`, `server.py` attaches `qa_report`. Confirmed §8's renumber-before-save rule is load-bearing beyond duplicate zip entries: python-pptx caches a relationship's `target_ref` on first serialization, so saving `draft.pptx` *before* renumbering leaves the delivered deck's `presentation.xml.rels` pointing at pre-rename slide parts and it will not re-open |
