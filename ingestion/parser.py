"""
ingestion/parser.py — parse_pptx(bytes) -> list[SlideRow], content_hash(bytes) -> str.
Handles: empty notes → "", missing title → None, image-only slides, zero-slide decks,
per-slide error isolation.
...
# content_hash enables nightly diff-sync; used only to detect changes before re-writing to S3.
...
# produces one SlideRow per slide so the LLM can identify which pages are relevant
    ...
    for i, slide in enumerate(prs.slides, start=1):
        try:
            ...
            cur_slide = SlideRow(
                ...
                body_text=body_text,
                speaker_notes=(
                    _clean(slide.notes_slide.notes_text_frame.text)
